...
9. 支持同时连接多个服务端，可以容灾和负载均衡；固定虚拟 IP（支持 CIDR，纯 IP 默认 `/24`）时本地网络立即启动，服务端在后台连接
10. 支持完全无服务器组网：配置固定虚拟 IP 和至少一个 `peer_address` 种子后，节点通过带身份的直连握手、Gossip 公告和按需探测形成多跳连通图
