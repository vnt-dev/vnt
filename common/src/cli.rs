...
pub fn parse_args_config() -> anyhow::Result<Option<(Config, Vec<String>, bool)>> {
    ...
    if language.starts_with("zh") {
        ...
        opts.optopt("k", "", "Network token", "<token>");
        opts.optopt("n", "", "Network name", "<name>");
        opts.optopt("d", "", "Device id", "<id>");
        opts.optflag("c", "", "Close interactive command");
        opts.optopt(
            "s",
            "",
            "registration and relay server addresses",
            "<server>",
        );
        opts.optmulti("e", "", "STUN servers", "<stun-server>");
        opts.optflag("a", "", "Tap mode");
        opts.optopt(
            "",
            "nic",
            "Virtual network adapter name, required when using Tap mode on Windows",
            "<tun0>",
        );
        opts.optmulti(
            "i",
            "",
            "Configuring inbound point-to-site (IP proxy) usage",
            "<in-ip>",
        );
        opts.optmulti(
            "o",
            "",
            "Configuring outbound point-to-site usage",
            "<out-ip>",
        );
        opts.optopt("w", "", "Client encryption", "<password>");
        opts.optflag("W", "", "Server-side encryption");
        opts.optopt("u", "", "Customize MTU (default is 1430)", "<mtu>");
        opts.optopt("", "ip", "Specify virtual IP", "<ip>");
        opts.optflag("", "relay", "Use server-side forwarding only");
        opts.optopt(
            "",
            "par",
            "Concurrency level (must be a positive integer)",
            "<parallel>",
        );
        opts.optopt("", "model", "Encryption mode", "<model>");
        opts.optflag("", "finger", "Fingerprint verification");
        opts.optopt("", "punch", "Value can be either IPv4 or IPv6", "<punch>");
        opts.optopt("", "ports", "Listening port", "<port,port>");
        opts.optflag("", "cmd", "Enable window input");
        opts.optflag("", "no-proxy", "Disable built-in proxy");
        opts.optflag("", "first-latency", "Priority delay");
        opts.optopt("", "use-channel", "Use channel relay/p2p", "<use-channel>");
        opts.optopt("", "packet-loss", "Packet loss rate", "<packet-loss>");
        opts.optopt("", "packet-delay", "Latency", "<packet-delay>");
        opts.optmulti("", "dns", "dns", "<dns>");
        opts.optmulti("", "mapping", "mapping", "<mapping>");
        opts.optmulti("", "vnt-mapping", "vnt-mapping", "<mapping>");
        opts.optopt("f", "", "Configuration file", "<conf>");
        opts.optopt("", "compressor", "Compression algorithm", "<lz4>");
        opts.optflag("", "disable-stats", "Disable traffic statistics");
        //"后台运行时,查看其他设备列表"
        opts.optflag("", "add", "When running in the background, add an address");
        opts.optflag(
            "",
            "list",
            "When running in the background, view the list of other devices",
        );
        opts.optflag(
            "",
            "all",
            "When running in the background, view comprehensive information about other devices",
        );
        opts.optflag(
            "",
            "info",
            "When running in the background, view information about the current device",
        );
        opts.optflag(
            "",
            "route",
            "When running in the background, view the data forwarding path",
        );
        opts.optflag(
            "",
            "chart_a",
            "When running in the background, view traffic statistics",
        );
        opts.optopt(
            "",
            "chart_b",
        ...
        opts.optflag("", "stop", "Stop running in the background");
